...
class TestDflashVerifyGraphSamplingPolicy(CustomTestCase):
    def test_greedy_argmax_graph_rejects_non_greedy_batches(self):
        from sglang.srt.environ import envs
        from sglang.srt.speculative.dflash_info import (
            _apply_dflash_verify_graph_sampling_policy,
            _is_dflash_verify_graph_sampling_compatible,
        )

        greedy = SimpleNamespace(is_all_greedy=True)
        sampling = SimpleNamespace(is_all_greedy=False)

        with envs.SGLANG_DFLASH_GREEDY_VERIFY_ARGMAX.override(False):
            self.assertTrue(
                _is_dflash_verify_graph_sampling_compatible(sampling)
            )
            batch = SimpleNamespace(
                sampling_info=sampling, disable_decode_cuda_graph=True
            )
            self.assertTrue(_apply_dflash_verify_graph_sampling_policy(batch))
            self.assertFalse(batch.disable_decode_cuda_graph)
        with envs.SGLANG_DFLASH_GREEDY_VERIFY_ARGMAX.override(True):
            self.assertTrue(_is_dflash_verify_graph_sampling_compatible(None))
            self.assertTrue(_is_dflash_verify_graph_sampling_compatible(greedy))
            self.assertFalse(
                _is_dflash_verify_graph_sampling_compatible(sampling)
            )
            batch = SimpleNamespace(
                sampling_info=sampling, disable_decode_cuda_graph=False
            )
            self.assertFalse(_apply_dflash_verify_graph_sampling_policy(batch))
            self.assertTrue(batch.disable_decode_cuda_graph)
