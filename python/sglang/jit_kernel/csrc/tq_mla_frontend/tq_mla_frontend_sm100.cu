...
__device__ __forceinline__ void write_cache_row(
    ...
    const uint8_t* storage_code_lut, uint8_t* packed_cache,
    __nv_bfloat16* scale_cache, uint8_t* rope_cache, int32_t* fault_status,
    const int token, const int64_t pool_size, const float scale_multiplier,
    const int lane) {
...
#pragma unroll
  ...
  if (lane == 0) {
    ...
    scale_cache[slot] = __float2bfloat16_rn(scale);
  }
...
template <int kWarps, bool kRotationFused, bool kStrict>
__global__ void tq_mla_frontend_kernel(
    const __nv_bfloat16* query_latent, const __nv_bfloat16* query_rope,
    const __nv_bfloat16* cache_latent, const __nv_bfloat16* cache_rope,
    const int64_t* locations, const float* signs1, const float* signs2,
    const float* boundaries, const float* quant_centroids,
    const uint8_t* storage_code_lut, uint8_t* query_out,
  ...
  if (warp == 0) {
    write_cache_row<kStrict>(cache_latent, cache_rope, locations, signs1,
                             signs2, boundaries, quant_centroids,
                             storage_code_lut, packed_cache, scale_cache,
                             rope_cache, fault_status, token, pool_size,
                             scale_multiplier, lane);
  }
}
...
void launch_frontend(
  ...
  tq_mla_frontend_kernel<kWarps, kRotationFused, kStrict>
      <<<grid, block, 0, stream>>>(
...
void tq_mla_frontend_out(
  ...
  TORCH_CHECK(properties->major == 10 && properties->minor == 0,
              "H41 native frontend requires SM100; got compute capability ",
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("tq_mla_frontend_out", &tq_mla_frontend_out,
             "H41 SM100 TurboQuant MLA query and writer front end");
}
