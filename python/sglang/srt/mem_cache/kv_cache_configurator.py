...
class KVCacheConfigurator:
    ...
    def _build_mla_turboquant_kv_pool(
        ...
        if is_native_e2m1_recip_bf16_mla_kv_cache_dtype(
            ...
            return MLATokenToKVPoolNativeE2M1RecipBF16(
                max_total_num_tokens,
        ...
        *,
        ...
        if turboquant_config is not None:
            assert not self.is_hybrid_swa_compress, (
                "TurboQuant does not yet support compressed hybrid-SWA models "
                "with distinct SWA head dimensions."
            )
            assert not self.post_capture_kv_active, (
                "TurboQuant does not yet support post-capture KV backing."
            )
            k_bits, v_bits, uniform = turboquant_config
            swa_pool_class = MHATokenToKVPoolTurboQuant
            kwargs.update(
                turboquant_bits=k_bits,
                turboquant_k_bits=k_bits,
                turboquant_v_bits=v_bits,
    ...
    def _build_mha_kv_pool(
        ...
        if turboquant_config is not None:
            ...
            pool_cls = MHATokenToKVPoolTurboQuant
