...
def test_check_finished_ignores_token_stop_before_min_new_tokens():
    req = _req(
        _sampling_params(
            max_new_tokens=8,
            min_new_tokens=2,
...
def test_check_finished_skips_early_stop_inside_speculative_acceptance():
    req = _req(
        _sampling_params(
            max_new_tokens=8,
            min_new_tokens=2,
...
def test_check_finished_does_not_carry_early_stop_to_later_token():
    req = _req(
        _sampling_params(
            max_new_tokens=8,
            min_new_tokens=2,
...
def test_check_finished_ignores_string_stop_before_min_new_tokens():
    req = _req(
        _sampling_params(
            max_new_tokens=8,
            min_new_tokens=2,
