...
from sglang.srt.speculative.dflash_utils import (
    ...
    compute_dflash_greedy_accept_output_inplace,
...
_DFLASH_CPU_COPY_STREAMS: dict[str, torch.cuda.Stream] = {}
...
def _get_cpu_copy_stream(device: torch.device | str) -> torch.cuda.Stream:
    device_str = str(device)
    stream = _DFLASH_CPU_COPY_STREAMS.get(device_str)
    if stream is None:
        stream = torch.get_device_module(device_str).Stream()
        _DFLASH_CPU_COPY_STREAMS[device_str] = stream
    return stream
...
class DFlashWorkerV2(DFlashWorker):
    ...
    def _make_next_draft_input_decode(
        ...
        *,
        ...
        draft_seq_lens: Optional[torch.Tensor] = None,
        draft_seq_lens_cpu: Optional[torch.Tensor] = None,
        draft_seq_lens_cpu_ready: Optional[torch.cuda.Event] = None,
    ) -> DFlashDraftInputV2:
        ...
        return DFlashDraftInputV2(
            ...
            draft_seq_lens_cpu_ready=draft_seq_lens_cpu_ready,
        )

    def _stage_seq_lens_cpu(
        self, seq_lens: torch.Tensor, bs: int
    ) -> tuple[Optional[torch.Tensor], Optional[torch.cuda.Event]]:
        if self._draft_seq_lens_cpu_buf is None or not seq_lens.is_cuda:
            return None, None

        dst = self._draft_seq_lens_cpu_buf[:bs]
        copy_stream = _get_cpu_copy_stream(seq_lens.device)
        current_stream = torch.get_device_module(seq_lens.device).current_stream()
        copy_stream.wait_stream(current_stream)
        ready = torch.get_device_module(seq_lens.device).Event()
        with torch.get_device_module(seq_lens.device).stream(copy_stream):
            dst.copy_(seq_lens.to(torch.int32), non_blocking=True)
            ready.record()
        return dst, ready
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        with _profile_range("dflash_v2.prepare_draft_block"):
            ...
            if not self._use_triton_prepare_block:
                ...
                verify_out_cache_loc_2d.copy_(
                    verify_out_cache_loc.view(bs, int(self.block_size))
                )
        ...
        with _profile_range("dflash_v2.prepare_draft_cache"):
            if self.use_compact_draft_cache:
                ...
                if (
                    ...
                    and int(draft_input.draft_seq_lens_cpu.numel()) >= bs
                ):
                    draft_input.wait_draft_seq_lens_cpu()
                    seq_lens_cpu = draft_input.draft_seq_lens_cpu[:bs]
                elif draft_input.new_seq_lens_cpu is not None:
                    draft_input.wait_new_seq_lens_cpu()
                    seq_lens_cpu.copy_(
                        self._compute_compact_draft_seq_lens_cpu(
                            draft_input.new_seq_lens_cpu[:bs]
                        )
                    )
                else:
                    seq_lens_cpu.copy_(
                ...
                if model_worker_batch.seq_lens_cpu is not None:
                    if model_worker_batch.seq_lens_cpu.dtype == torch.int32:
                        ...
                        seq_lens_cpu.copy_(
                            model_worker_batch.seq_lens_cpu.to(torch.int32)
                        )
                else:
        ...
        with _profile_range("dflash_v2.draft_sample"):
        ...
        with _profile_range("dflash_v2.target_verify"):
        ...
        with _profile_range("dflash_v2.accept"):
            ...
            if (
                ...
                if has_argmax_token_ids:
                    target_predict = next_token_logits.view(bs, int(self.block_size))
                else:
                    target_predict = torch.argmax(next_token_logits, dim=-1).view(
                        bs, int(self.block_size)
                    )
                assert self._draft_accept_len_buf is not None
                assert self._draft_commit_lens_buf is not None
                assert self._draft_bonus_buf is not None
                assert self._draft_out_tokens_buf is not None
                assert self._draft_new_seq_lens_buf is not None
                accept_len = self._draft_accept_len_buf[:bs]
                commit_lens = self._draft_commit_lens_buf[:bs]
                bonus = self._draft_bonus_buf[:bs]
                out_tokens = self._draft_out_tokens_buf[:bs]
                new_seq_lens = self._draft_new_seq_lens_buf[:bs]
                if not compute_dflash_greedy_accept_output_inplace(
                    candidates=candidates,
        ...
        # Materialize the full fixed verify block. Slots past commit_lens are not part
        # of the committed prefix and are overwritten before they can become visible.
        # Keeping this dense lets the fused KV materializer stay on the hot path.
        with _profile_range("dflash_v2.materialize_draft_kv"):
            self._append_target_hidden_to_draft_kv_by_loc(
        ...
        with _profile_range("dflash_v2.finish"):
            # Avoid copying large hidden-state buffers to CPU in overlap scheduling.
            logits_output.hidden_states = None
            ...
            if self.use_compact_draft_cache:
                assert self._draft_prefix_lens_buf is not None
                assert self._draft_block_end_buf is not None
                next_draft_seq_lens = self._draft_prefix_lens_buf[:bs]
                next_block_end = self._draft_block_end_buf[:bs]
                if not self._compute_compact_draft_seq_lens_out(
                    seq_lens=new_seq_lens,
                    draft_seq_lens=next_draft_seq_lens,
                    block_end=next_block_end,
                ):
                    next_draft_seq_lens.copy_(
                        self._compute_compact_draft_seq_lens(new_seq_lens)
                    )
                (
                    next_draft_seq_lens_cpu,
                    next_draft_seq_lens_cpu_ready,
                ) = self._stage_seq_lens_cpu(next_draft_seq_lens, bs)

            next_draft_input = self._make_next_draft_input_decode(
                verified_id=bonus,
                new_seq_lens=new_seq_lens,
                draft_seq_lens=next_draft_seq_lens,
                draft_seq_lens_cpu=next_draft_seq_lens_cpu,
                draft_seq_lens_cpu_ready=next_draft_seq_lens_cpu_ready,
            )
            verify_done = torch.get_device_module(device).Event()
            verify_done.record()
            next_draft_input.verify_done = verify_done
