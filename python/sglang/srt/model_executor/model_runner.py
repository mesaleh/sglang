...
class ModelRunner:
    ...
    def _record_kv_cache_dtype(self, resolved: str) -> None:
        # Load-time resolution transition: the weight- or role-resolved
        # kv-cache dtype is declared into the flags tier; the dual-apply inside
        # the helper replaces the legacy in-place write. Mock runners whose
        # server_args is not the published object keep the plain write.
    ...
    def configure_kv_cache_dtype(self):
        ...
        (
            self.turboquant_k_bits,
            self.turboquant_v_bits,
            self.turboquant_uniform,
        ) = turboquant_config
        self.turboquant_bits = self.turboquant_k_bits
        ...
        if not self.use_mla_backend:
            if (
                ...
                self.server_args.override(
                    "ModelRunner.configure_kv_cache_dtype.turboquant",
                    prefill_attention_backend="triton",
                    decode_attention_backend="triton",
                )
                logger.info(
                    "TurboQuant: overriding prefill/decode attention backends "
                    "%s/%s -> triton/triton (packed MHA kernels).",
                    previous_prefill,
                    previous_decode,
                )
        else:
            logger.info(
                "TurboQuant+MLA: keeping decode-attention-backend=%s; "
                "the MLA pool dequantizes on read.",
                self.server_args.decode_attention_backend or "default",
            )

        has_fused_decode = self.turboquant_k_bits in (
            2,
            4,
        ) and self.turboquant_v_bits in (2, 4)
        if (
            not self.use_mla_backend
            and not has_fused_decode
            and not self.server_args.disable_cuda_graph
        ):
            logger.warning(
                "TurboQuant K=%dbit V=%dbit has no fused decode kernel; "
        ...
        logger.info(
            "TurboQuant K=%dbit V=%dbit KV cache enabled",
            self.turboquant_k_bits,
            self.turboquant_v_bits,
        )
    ...
    def _maybe_fuse_tq_output_rotation(self):
        ...
        for _, mod in self.model.named_modules():
            if hasattr(mod, "o_proj") and hasattr(mod.o_proj, "weight"):
                ...
                if w.shape[1] % tq_cfg.head_dim == 0 and n_heads > 0:
                    fuse_candidates.append((w, n_heads))
                else:
                    skipped_other += 1
        ...
        skipped_layers = skipped_quantized + skipped_other
        fused_all = fuse_turboquant_output_rotation_weights(
            tq_cfg,
            fuse_candidates,
        ...
        if skipped_layers:
            logger.info(
                "TurboQuant: leaving all %d eligible o_proj layers unchanged "
                "because %d layers cannot be fused; using runtime inverse "
        ...
        if fused_all:
            logger.info(
                "TurboQuant: fused inverse rotation into %d o_proj layers",
                len(fuse_candidates),
            )
