...
class DFlashDraftInputV2(SpecInput):
    ...
    def bonus_tokens(self) -> torch.Tensor:
        """Compatibility alias used by overlap FutureMap.

        DFLASH carries the next accepted/verified seed token as `verified_id`.
        The existing spec-v2 FutureMap stores the same concept under the
        generic EAGLE-style `bonus_tokens` field, so keep both names tied to
        the same tensor.
        """
        return self.verified_id

    @bonus_tokens.setter
    def bonus_tokens(self, value: torch.Tensor) -> None:
    ...
    def wait_new_seq_lens_cpu(self) -> None:
        if self.new_seq_lens_cpu_ready is not None:
            self.new_seq_lens_cpu_ready.synchronize()
            self.new_seq_lens_cpu_ready = None

    def wait_draft_seq_lens_cpu(self) -> None:
        if self.draft_seq_lens_cpu_ready is not None:
            self.draft_seq_lens_cpu_ready.synchronize()
            self.draft_seq_lens_cpu_ready = None
    ...
    def prepare_for_decode(self, batch: ScheduleBatch):
        ...
        cur_kv_lens_list = [0] * bs
        nxt_kv_lens_list = [0] * bs
        num_needed_tokens = 0
        for i, req in enumerate(batch.reqs):
            cur = int(req.kv_allocated_len)
            # Pre-claim one bonus slot, matching the spec-v2 output processor.
            # `kv_committed_len` may lag the GPU seq_lens by one overlapped result;
            # the 2*block headroom absorbs that lag without a GPU->CPU sync.
            nxt = max(cur, int(req.kv_committed_len) + 1 + 2 * block_size)
            cur_kv_lens_list[i] = cur
            nxt_kv_lens_list[i] = nxt
        ...
        for req, new_alloc_len in zip(batch.reqs, nxt_kv_lens_list, strict=True):
        ...
        batch.seq_lens_sum = int(sum(nxt_kv_lens_list))
    ...
    def merge_batch(self, spec_info: "DFlashDraftInputV2"):
        if self.future_indices is not None:
            ...
            if (
                ...
                and spec_info.new_seq_lens_cpu is not None
            ):
                self.wait_new_seq_lens_cpu()
                spec_info.wait_new_seq_lens_cpu()
                self.new_seq_lens_cpu = torch.cat(
                    [self.new_seq_lens_cpu, spec_info.new_seq_lens_cpu], dim=0
                )
                self.new_seq_lens_cpu_ready = None
            else:
                ...
                self.new_seq_lens_cpu_ready = None
            if (
                self.draft_seq_lens is not None
                and spec_info.draft_seq_lens is not None
            ):
            ...
            if (
                self.draft_seq_lens_cpu is not None
                and spec_info.draft_seq_lens_cpu is not None
            ):
                self.wait_draft_seq_lens_cpu()
                spec_info.wait_draft_seq_lens_cpu()
                self.draft_seq_lens_cpu = torch.cat(
                    [self.draft_seq_lens_cpu, spec_info.draft_seq_lens_cpu], dim=0
                )
                self.draft_seq_lens_cpu_ready = None
            else:
                self.draft_seq_lens_cpu = None
                self.draft_seq_lens_cpu_ready = None
            return
        ...
        if self.draft_seq_lens is not None or spec_info.draft_seq_lens is not None:
            if self.draft_seq_lens is None or spec_info.draft_seq_lens is None:
                self.draft_seq_lens = None
            else:
                self.draft_seq_lens = torch.cat(
                    [self.draft_seq_lens, spec_info.draft_seq_lens], dim=0
                )
        if self.new_seq_lens_cpu is not None or spec_info.new_seq_lens_cpu is not None:
            if self.new_seq_lens_cpu is None or spec_info.new_seq_lens_cpu is None:
                self.new_seq_lens_cpu = None
                self.new_seq_lens_cpu_ready = None
            else:
                self.wait_new_seq_lens_cpu()
        ...
        if (
            self.draft_seq_lens_cpu is not None
            or spec_info.draft_seq_lens_cpu is not None
        ):
            if (
                self.draft_seq_lens_cpu is None
                or spec_info.draft_seq_lens_cpu is None
            ):
                self.draft_seq_lens_cpu = None
                self.draft_seq_lens_cpu_ready = None
            else:
