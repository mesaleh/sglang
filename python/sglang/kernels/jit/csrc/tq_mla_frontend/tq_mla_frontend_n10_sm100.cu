...
#pragma unroll
    ...
    if (item < 8) {
      ...
      candidate.data1 |= static_cast<uint64_t>(bin) << ((item - 8) * 8);
    }
...
__device__ __forceinline__ void store_cache_row(
    const LaneCandidate& candidate, const uint8_t* storage_codes,
...
#pragma unroll
    for (int item = 0; item < 2; ++item) {
      const int even_index = chunk * 4 + 2 * item;
      const int odd_index = even_index + 1;
      const uint64_t even_data =
          even_index < 8 ? candidate.data0 : candidate.data1;
      const uint64_t odd_data =
          odd_index < 8 ? candidate.data0 : candidate.data1;
      const int even_shift =
          (even_index < 8 ? even_index : even_index - 8) * 8;
      const int odd_shift =
          (odd_index < 8 ? odd_index : odd_index - 8) * 8;
      const uint8_t even_bin =
          static_cast<uint8_t>(even_data >> even_shift);
      const uint8_t odd_bin = static_cast<uint8_t>(odd_data >> odd_shift);
      const uint8_t even = storage_codes[even_bin];
      const uint8_t odd = storage_codes[odd_bin];
...
__device__ __forceinline__ void tq_mla_frontend_body(
  ...
  if (warp < kQueryHeads) {
    ...
    candidate = compute_cache_row<kApplyRope>(
        cache_latent, cache_rope, signs1, signs2, boundaries, levels, token,
        grid, cache_latent_stride_t, cache_latent_stride_h,
...
#pragma unroll
  ...
  if (warp < kQueryHeads) {
    ...
    store_cache_row(candidate, storage_codes, packed_cache, scale_cache,
                    rope_cache, slot, packed_stride_s, packed_stride_h,
...
#define LAUNCH_FRONTEND(ROTATION, STRICT)                                \
  launch_frontend<ROTATION, STRICT, index_t, kApplyRope>(                 \
      ...
      signs1, signs2, boundaries, levels, storage_codes,                 \
      query_latent_out, query_rope_out, packed_cache, scale_cache,       \
...
void tq_mla_cache_writer_out(
  ...
  validate_cache_contract(cache_latent, cache_rope, locations, signs1, signs2,
                          ...
                          scale_cache, rope_cache, fault_status, zero_count,
                          grid);
  const torch::Device device = cache_latent.device();
