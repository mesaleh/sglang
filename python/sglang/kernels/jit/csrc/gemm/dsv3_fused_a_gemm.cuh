...
struct DSV3FusedAGemmKernel {
  ...
  run(const tvm::ffi::TensorView mat_a, const tvm::ffi::TensorView mat_b, const tvm::ffi::TensorView output) {
    ...
    if (forced_tile_n == 1 && num_tokens <= 1) {
      invokeFusedAGemm<bf16_t, kHdIn, kHdOut, 1, kTileM, kUsePDL>(out_ptr, a_ptr, b_ptr, num_tokens, dev);
    } else if (forced_tile_n == 2 && num_tokens <= 2) {
      invokeFusedAGemm<bf16_t, kHdIn, kHdOut, 2, kTileM, kUsePDL>(out_ptr, a_ptr, b_ptr, num_tokens, dev);
    } else if (forced_tile_n == 4 && num_tokens <= 4) {
      invokeFusedAGemm<bf16_t, kHdIn, kHdOut, 4, kTileM, kUsePDL>(out_ptr, a_ptr, b_ptr, num_tokens, dev);
