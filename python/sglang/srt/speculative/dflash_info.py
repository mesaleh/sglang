...
def _is_dflash_verify_graph_sampling_compatible(sampling_info) -> bool:
    return (
        not envs.SGLANG_DFLASH_GREEDY_VERIFY_ARGMAX.get()
        or sampling_info is None
        or sampling_info.is_all_greedy
    )


def _apply_dflash_verify_graph_sampling_policy(forward_batch: ForwardBatch) -> bool:
    compatible = _is_dflash_verify_graph_sampling_compatible(
        forward_batch.sampling_info
    )
    forward_batch.disable_decode_cuda_graph = not compatible
    return compatible
...
class DFlashVerifyInput(SpecInput):
    ...
    def prepare_for_verify(
        ...
        _apply_dflash_verify_graph_sampling_policy(verify_forward_batch)
